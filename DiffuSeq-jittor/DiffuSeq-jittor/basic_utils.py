...
class myTokenizer():
    ...
    def __init__(self, args):
        if args.vocab == 'bert':
            ...
            args.config_name = 'models/bert-base-uncased'
...
def load_model_emb(args, tokenizer):
    ...
    model = jt.nn.Embedding(tokenizer.vocab_size, args.hidden_dim)
    path_save = '{}/random_emb.jittor'.format(args.checkpoint_path)
    ...
    if int(os.environ.get('LOCAL_RANK', 0)) == 0:
        if os.path.exists(path_save):
            ...
            jt.nn.init.kaiming_normal_(model.weight)
