import jittor as jt
from numpy import inf

def get_knn(model_emb: jt.Var, text_emb: jt.Var, dist='cos'):
    if dist == 'cos':
        adjacency = model_emb @ text_emb.transpose((1, 0))
    elif dist == 'l2':
        adjacency = model_emb.unsqueeze(1).expand(-1, text_emb.size(0), -1) - text_emb.unsqueeze(0).expand(
            model_emb.size(0), -1, -1)
        adjacency = -jt.norm(adjacency, dim=-1)
    topk_out = jt.topk(adjacency, k=6, dim=0)
    return topk_out[0], topk_out[1]

def get_efficient_knn(model_emb: jt.Var, text_emb: jt.Var):
    emb_norm = (model_emb**2).sum(-1).view(-1, 1)
    text_emb_t = jt.transpose(text_emb.view(-1, text_emb.size(-1)), 0, 1)
    arr_norm: jt.Var = (text_emb**2).sum(-1).view(-1, 1)
    dist = emb_norm + arr_norm.transpose(0, 1) - 2.0 * jt.matmul(model_emb, text_emb_t)
    dist = jt.clamp(dist, 0.0, inf)
    topk_out = jt.topk(-dist, k=1, dim=0)
    return topk_out[0], topk_out[1]

def rounding_func(text_emb_lst, model, tokenizer, emb_scale_factor=1.0):
    decoded_out_lst = []
    model_emb = model.weight  # input_embs
    down_proj_emb2 = None
    dist = 'l2'
    for text_emb in text_emb_lst:
        text_emb = jt.Var(text_emb)
        if len(text_emb.shape) > 2:
            text_emb = text_emb.view(-1, text_emb.size(-1))
        val, indices = get_knn((down_proj_emb2 if dist == 'cos' else model_emb),
                               text_emb, dist)
        decoded_out_lst.append(tokenizer.decode_token(indices[0]))
    return decoded_out_lst

def compute_logp(args, model, x: jt.Var, input_ids: jt.Var):
    word_emb = model.weight
    sigma = 0.1
    if args.model_arch == '1d-unet':
        x = x.permute(0, 2, 1)
    bsz, seqlen, dim = x.shape
    x_flat = x.reshape(-1, x.size(-1)).unsqueeze(0)
    word_emb_flat = word_emb.unsqueeze(1)
    diff: jt.Var = (x_flat - word_emb_flat) ** 2
    logp_expanded = -diff.sum(dim=-1) / (2 * sigma ** 2)
    logp_expanded = logp_expanded.permute((1, 0))
    ce = jt.nn.CrossEntropyLoss()
    loss = ce(logp_expanded, input_ids.view(-1)).view(bsz, seqlen)
    return loss

def get_weights(model, args):
    if hasattr(model, 'transformer'):
        input_embs = model.transformer.wte  # input_embs
        down_proj = model.down_proj
        model_emb = down_proj(input_embs.weight)
        print(model_emb.shape)
        model = jt.nn.Embedding(model_emb.size(0), model_emb.size(1))
        print(args.emb_scale_factor)
        model.weight.data = model_emb * args.emb_scale_factor
    
    elif hasattr(model, 'weight'):
        pass
    else:
        assert NotImplementedError
    
    model.weight.stop_grad()
    return model

def denoised_fn_round(args, model, text_emb: jt.Var, t):
    model_emb = model.weight
    old_shape = text_emb.shape
    if len(text_emb.shape) > 2:
        text_emb = text_emb.reshape(-1, text_emb.size(-1))
    val, indices = get_efficient_knn(model_emb, text_emb)
    rounded_tokens = indices[0]
    new_embeds = model(rounded_tokens).view(old_shape)
    return new_embeds