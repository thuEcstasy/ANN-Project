...
def betas_for_alpha_bar_left(num_diffusion_timesteps, alpha_bar, max_beta=0.999):
    ...
    for i in range(num_diffusion_timesteps-1):
        ...
        betas = jt.concat([betas, jt.Var(min(1 - alpha_bar(t2) / alpha_bar(t1), max_beta))])
    return betas
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        betas = jt.Var(betas).cast(jt.float64)
        if len(betas.shape) == 2:
            if betas.shape[1] == 1:
                betas = betas.reshape([betas.shape[0]])
        self.betas = betas
        # print(betas.shape)
        # print(betas)
        ...
        self.alphas_cumprod_prev = jt.concat([jt.Var(1.0), self.alphas_cumprod[:-1]])
        self.alphas_cumprod_next = jt.concat([self.alphas_cumprod[1:], jt.Var(0.0)])
    ...
    def q_sample(self, x_start, t, noise=None, mask=None):
        ...
        if mask is None:
            ...
            ret = jt.where(mask==0, x_start, x_t)
            return ret
    ...
    def _get_x_start(self, x_start_mean, std):
        ...
        :return: x_0
        '''
        x_start_mean = jt.from_torch(x_start_mean.detach())
    ...
    def _token_discrete_loss(self, x_t, get_logits, input_ids, mask=None, truncate=False, t=None):
        ...
        t2 = time.time()
        print('     T1 -- T2,', t2-t1,'seconds')
        if mask is not None:
            decoder_nll *= mask
        decoder_nll = decoder_nll.data
        
        if mask is not None:
            mask = mask.data
            decoder_nll = decoder_nll.sum(-1)/(mask.sum(-1) + 1e-8)
        else:
            decoder_nll = decoder_nll.mean(-1)
        t3 = time.time()
        print('     T2 -- T3,', t3-t2,'seconds')
        decoder_nll = jt.Var(decoder_nll)
        return decoder_nll
    ...
    def training_losses_seq2seq(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        t1 = time.time()
        print('get embeds', t1-t0, 'seconds')
    ...
    def __init__(self, use_timesteps, **kwargs):
        ...
        for i, alpha_cumprod in enumerate(base_diffusion.alphas_cumprod):
            if i in self.use_timesteps:
                new_betas = jt.concat([new_betas, jt.Var((1 - alpha_cumprod / last_alpha_cumprod))])
                last_alpha_cumprod = alpha_cumprod
                self.timestep_map.append(i)
        kwargs["betas"] = jt.Var(new_betas)
