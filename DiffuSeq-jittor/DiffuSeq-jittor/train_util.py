...
import jittor as jt
from added_jittor_util import clip_grad_norm_
...
class TrainLoop:
    ...
    def run_loop(self):
        while (
            ...
            _t_ = time.time() - _t_
            print('Run one step time used:', _t_, 'seconds')
            ...
            if self.eval_data is not None and self.step % self.eval_interval == 0:
                ...
                logger.dumpkvs()
                print('finished dumping kvs', flush=True)
    ...
    def run_step(self, batch, cond):
        ...
        t2 = time.time()
        print('forward_backward cost', t2-t1, 'seconds')
        if self.use_fp16:
            self.optimize_fp16()
        else:
            self.optimize_normal()
        t3 = time.time()
        print('optimize_normal cost', t3-t2, 'seconds')
        self.log_step()
        t4 = time.time()
        print('log_step cost', t4-t3, 'seconds')
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            if last_batch or not self.use_ddp:
                ...
                with self.ddp_model.no_sync():
                    losses = compute_losses()
            t2 = time.time()
            print('compute losses cost', t2-t1, 'seconds')
            ...
            if False:
                ...
                self.opt.step(loss)
            t3 = time.time()
            print('opt step loss cost', t3-t2, 'seconds')
    ...
    def _log_grad_norm(self):
        ...
        logger.logkv_mean("grad_norm", jt.sqrt(sqsum))
